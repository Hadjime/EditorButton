...
public class EditorButton : Editor
{
	...
	void DrawButtonforMethod(MonoBehaviour target, MethodInfo methodInfo, EditorButtonState state) {
		...
		if (methodInfo.GetParameters().Length > 0) state.opened = EditorGUI.Foldout (foldoutRect, state.opened, "");
	...
	string MethodDisplayName(MethodInfo method = null) {
		StringBuilder sb = new StringBuilder ();

        EditorButtonAttribute eba = method.GetCustomAttributes(typeof(EditorButtonAttribute), false)[0] as EditorButtonAttribute;

        if (eba.text != "") sb.Append(eba.text + " ");
        if (eba.showInfo)
        {
